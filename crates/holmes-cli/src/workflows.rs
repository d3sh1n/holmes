...
async fn run_agent_turn(
    ...
    session.messages.push(Message::user(format!("[当前阶段] {}", stage_prompt)));
    ...
    loop {
        if !budget.consume() {
            session.messages.push(Message::user("[阶段完成] 已达到本阶段最大轮次。"));
            return Ok(());
        }

        let response = llm.chat_completion(&session.messages, &tools, "attack_agent")
            .await
            ...
            for call in &response.tool_calls {
                let verdict = {
                    let state = holmes_core::state::AttackState::new(
                        "unknown".into(), String::new(), String::new(), String::new(), vec![]
                    );
...
impl Workflow for ReconWorkflow {
    fn name(&self) -> &str { "recon" }
    fn description(&self) -> &str {
        "信息收集：端口扫描、子域名枚举、目录爆破、技术栈识别。当需要了解目标时使用。"
    }
    async fn forward(&self, session: &mut RuntimeSession) -> Result<(), WorkflowError> {
        run_agent_turn(session, "你正在进行信息收集阶段。使用工具扫描目标，了解攻击面。", &self.llm, &self.registry, &self.guards).await
    }
}
...
impl Workflow for AnalysisWorkflow {
    ...
    async fn forward(&self, session: &mut RuntimeSession) -> Result<(), WorkflowError> {
        run_agent_turn(session, "你正在进行分析阶段。深入分析目标，识别漏洞和弱点。", &self.llm, &self.registry, &self.guards).await
    }
}
...
impl Workflow for ExploitWorkflow {
    fn name(&self) -> &str { "exploit" }
    fn description(&self) -> &str {
        "漏洞利用：漏洞利用、权限提升、横向移动、Payload构造。当需要执行攻击时使用。"
    }
    async fn forward(&self, session: &mut RuntimeSession) -> Result<(), WorkflowError> {
        run_agent_turn(session, "你正在进行漏洞利用阶段。利用已发现的漏洞，获取访问权限。", &self.llm, &self.registry, &self.guards).await
    }
}
...
impl Workflow for ReportWorkflow {
    ...
    async fn forward(&self, session: &mut RuntimeSession) -> Result<(), WorkflowError> {
        run_agent_turn(session, "你正在生成报告。整理所有发现，生成结构化报告。", &self.llm, &self.registry, &self.guards).await
    }
}
...
impl Workflow for ChatWorkflow {
    ...
    async fn forward(&self, session: &mut RuntimeSession) -> Result<(), WorkflowError> {
        run_agent_turn(session, "你是 Holmes。回答用户的问题，如果需要使用工具就使用。", &self.llm, &self.registry, &self.guards).await
    }
}
...
pub fn create_builtin_workflows(
    ...
    vec![
        Box::new(ChatWorkflow { llm: llm.clone(), registry: registry.clone(), guards: guards.clone() }),
        Box::new(ReconWorkflow { llm: llm.clone(), registry: registry.clone(), guards: guards.clone() }),
        Box::new(AnalysisWorkflow { llm: llm.clone(), registry: registry.clone(), guards: guards.clone() }),
        Box::new(ExploitWorkflow { llm: llm.clone(), registry: registry.clone(), guards: guards.clone() }),
        Box::new(ReportWorkflow { llm, registry, guards }),
    ]
}
