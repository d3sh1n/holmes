...
use crate::context_layer::{ContextLayer, CompromisedHostSummary, ReverseInsightSummary};
    ...
    pub fn generate(context: &ContextLayer, mode: &SessionMode) -> DashboardSnapshot {
        ...
        match mode {
            SessionMode::Pentest | SessionMode::SecurityResearch | SessionMode::Mixed => {
                if !context.attack_surface.hosts.is_empty() || !context.attack_surface.services.is_empty() {
                    sections.insert("attack_surface".into(), DashboardSection {
                        title: "攻击面".into(),
                        ...
                        item_count: context.attack_surface.hosts.len() + context.attack_surface.services.len(),
                    });
                }
                if let Some(ref topo) = context.network_topology {
                    sections.insert("network_topology".into(), DashboardSection {
                        title: "内网拓扑".into(),
                        content_summary: format!("子网: {}, 主机: {}, 信任路径: {}", topo.subnets.len(), topo.hosts.len(), topo.trust_paths.len()),
                        item_count: topo.hosts.len(),
                    });
                }
                if !context.compromised_hosts.is_empty() {
                    sections.insert("compromised".into(), DashboardSection {
                        title: "已控主机".into(),
                        content_summary: context.compromised_hosts.iter()
            ...
            SessionMode::CodeAudit => {
                if !context.code_patterns.is_empty() {
                    sections.insert("code_patterns".into(), DashboardSection {
                        title: "代码模式".into(),
                        content_summary: context.code_patterns.iter()
            ...
            SessionMode::Reverse => {
                if !context.reverse_insights.is_empty() {
                    sections.insert("reverse_insights".into(), DashboardSection {
                        title: "逆向发现".into(),
                        content_summary: context.reverse_insights.iter()
                            .map(|i| format!("{}: {} ({})", i.insight_type_str(), i.description, i.confidence))
                            .collect::<Vec<_>>().join("; "),
        ...
        if !context.vulnerabilities.is_empty() {
            sections.insert("vulnerabilities".into(), DashboardSection {
                title: "漏洞发现".into(),
                content_summary: context.vulnerabilities.iter()
        ...
        if !context.credentials.is_empty() {
            sections.insert("credentials".into(), DashboardSection {
                title: "凭据".into(),
        ...
        if context.stack.depth() > 0 {
            sections.insert("context".into(), DashboardSection {
                title: "当前上下文".into(),
        ...
        DashboardSnapshot { sections, timestamp: chrono::Utc::now() }
    }
}

impl CompromisedHostSummary {
    fn access_level_str(&self) -> &str {
        match self.access_level { AccessLevel::User => "User", AccessLevel::Root => "Root", AccessLevel::System => "SYSTEM", AccessLevel::DomainAdmin => "Domain Admin" }
    }
}
